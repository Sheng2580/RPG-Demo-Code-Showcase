...
- `Scripts/Framework/PoolManager.cs`：对象池管理，用于特效、UI、战斗对象复用。
- `Scripts/Framework/ResourceManager.cs`：Resources 资源加载入口。
- `Scripts/Framework/GameSceneManager.cs`：项目场景跳转封装，避免和 Unity 原生 `SceneManager` 混用。
...
代码来自原 Unity 项目的 `Assets/Scripts` 目录，已排除 `.meta`、场景、Prefab、材质、音频和 ScriptableObject 配置资产。部分脚本依赖 UnityEngine、Cinemachine、Input System 或项目中的 Prefab/配置资源，本仓库以代码阅读为主，不保证可单独编译运行。
